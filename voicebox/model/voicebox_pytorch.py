...
def mask_from_frac_lengths(
    ...
    lengths = (frac_lengths * seq_lens).long()
    max_start = seq_lens - lengths

    rand = torch.zeros_like(frac_lengths).float().uniform_(0, 1)
    start = (max_start * rand).clamp(min = 0)
    end = start + lengths

    seq_len = torch.max(seq_lens).item()
    #print(seq_len)
    #print(start)
    #print(end)
    ...
    def __init__(
        ...
        *,
        ...
        for _ in range(depth):
            self.dw_conv1d.append(nn.Conv1d(dim, dim, kernel_size, groups = groups, padding = kernel_size // 2))
            self.dw_conv1d.append(nn.GELU())
        ...
        *,
        ...
        for ind in range(depth):
            ...
            self.layers.append(nn.ModuleList([
                ...
                Attention(dim = dim, dim_head = dim_head, heads = heads, flash = attn_flash, dropout=attention_dropout),
                rmsnorm_klass(dim = dim),
                FeedForward(dim = dim, mult = ff_mult, dropout=activation_dropout)
            ]))
    ...
    def forward(
        ...
        *,
        ...
        if not exists(seq_lens):
            # print("seq_lens not exists")
            seq_lens = torch.zeros((batch, ), dtype=torch.long, device = x.device) + seq_len
        ...
        if not exists(mask):
            if coin_flip():
                frac_lengths = torch.zeros((batch,), device = self.device).float().uniform_(*self.frac_lengths_mask)
                mask = mask_from_frac_lengths(seq_lens, frac_lengths)
                # print("ccccc")
            else:
                mask = prob_mask_like((batch, seq_len), self.p_drop_prob, self.device)
                # print("ddddd")
        ...
        print(mask.shape)
        print(mask[0])
        torch.save(mask[0], "mask.pt")
        #print(mask[1])
        print(cond[0])
        torch.save(cond[0], "a.pt")
        """
        cond = cond * rearrange(~mask, '... -> ... 1')
        ...
        phoneme_emb = self.to_phoneme_emb(phoneme_ids)
        embed = torch.cat((x, cond, phoneme_emb), dim = -1)
        # print(embed.shape)
        # return
        x = self.to_embed(embed)
        # print(x.shape)
        ...
        # time_emb = repeat(time_emb, 'b c -> b t c', t = x.shape[1])
        # x = x + time_emb
        time_emb = repeat(time_emb, 'b c -> b 1 c')
        ...
        num = loss.sum()
        den = mask.sum().clamp(min=1e-5)
        loss1 = num / den

        # this is original per wav loss, may all mean loss is better
        # masked mean
        num = reduce(loss, 'b n -> b', 'sum')
        den = mask.sum(dim = -1).clamp(min = 1e-5)
        loss = num / den
